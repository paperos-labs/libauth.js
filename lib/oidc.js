  ...
  return function _oidc(pluginOpts) {
    ...
    async function generateAuthUrl(req, res, next) {
      ...
      let redirectUri =
        ...
        req.originalUrl ||
        req.url;
      let redirectHost = libauth.get(req, "redirectHost") || libOpts.issuer;

      // SECURITY check redirectUri
      if (!startsWithProto(redirectUri)) {
        if (!startsWithProto(redirectHost)) {
          redirectUri += "https://";
        }
        redirectHost += stripTrailingSlash(redirectHost);
        pluginOpts.redirectUri = `${redirectHost}${redirectUri}`;
      }
